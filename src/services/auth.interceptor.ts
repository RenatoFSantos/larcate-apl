...
export class AuthInterceptor implements HttpInterceptor {
  ...
  intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    return next.handle(authReq).pipe(
      catchError(error => {
        if (error instanceof HttpErrorResponse && !authReq.url.includes('users/auth')
        && !authReq.url.includes('users/refreshtoken') && error.status === 401 && !this.isRefreshing) {
      ...
      if (refreshToken) {
        return this.authService.newToken(refreshToken).pipe(
          switchMap((res: any) => {
            ...
            return next.handle(this.addTokenHeader(request, res.token.message.token));
          }),
          catchError((err) => {
            this.isRefreshing = false;
